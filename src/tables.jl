# This file is auto-generated by extractdata/geometry.jl; do not edit directly.

# Sequence of construction of atoms in residues
# The pattern (a, b, c, [d1, d2, ...]) indicates that atoms d1, d2, ... are attached to b
# The pattern (a, b, c, d, tf) indicates that atom d is attached to c; tf indicates whether the dihedral
#   formed by a-b-c-d is rotatable (true) or fixed (false)
# A reference atom prefixed with "-" or "+" (e.g. "-C", "+N") names that atom in the previous or
#   next residue rather than the current one, following the CHARMM convention.
const residue_build_sequence = Dict(
    "ALA" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "HB1", true),
            ("CA", "CB", "HB1", ["HB2", "HB3"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "ARG" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "NE", true),
            ("CG", "CD", "NE", ["HD2", "HD3"]),
            ("CG", "CD", "NE", "CZ", true),
            ("CD", "NE", "CZ", ["HE"]),
            ("CD", "NE", "CZ", "NH1", true),
            ("NE", "CZ", "NH1", ["NH2"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
            ("NE", "CZ", "NH2", "HH22", false),
            ("CZ", "NH2", "HH22", ["HH21"]),
            ("NE", "CZ", "NH1", "HH11", false),
            ("CZ", "NH1", "HH11", ["HH12"]),
        ],
    "ASN" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "OD1", true),
            ("CB", "CG", "OD1", ["ND2"]),
            ("CB", "CG", "ND2", "HD21", true),
            ("CG", "ND2", "HD21", ["HD22"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "ASP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "OD1", true),
            ("CB", "CG", "OD1", ["OD2"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "CALA" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("C", "CA", "CB", "HB1", true),
            ("CA", "CB", "HB1", ["HB2", "HB3"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CARG" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "NE", true),
            ("CG", "CD", "NE", ["HD2", "HD3"]),
            ("CG", "CD", "NE", "CZ", true),
            ("CD", "NE", "CZ", ["HE"]),
            ("CD", "NE", "CZ", "NH1", true),
            ("NE", "CZ", "NH1", ["NH2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("-C", "CA", "N", "H", false),
            ("NE", "CZ", "NH2", "HH22", false),
            ("CZ", "NH2", "HH22", ["HH21"]),
            ("NE", "CZ", "NH1", "HH11", false),
            ("CZ", "NH1", "HH11", ["HH12"]),
        ],
    "CASN" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "OD1", true),
            ("CB", "CG", "OD1", ["ND2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG", "ND2", "HD21", true),
            ("CG", "ND2", "HD21", ["HD22"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CASP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "OD1", true),
            ("CB", "CG", "OD1", ["OD2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CCYS" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("C", "CA", "CB", "SG", true),
            ("CA", "CB", "SG", ["HB2", "HB3"]),
            ("CA", "CB", "SG", "HG", true),
            ("-C", "CA", "N", "H", false),
        ],
    "CGLN" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "OE1", true),
            ("CG", "CD", "OE1", ["NE2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CG", "CD", "NE2", "HE21", true),
            ("CD", "NE2", "HE21", ["HE22"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CGLU" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "OE1", true),
            ("CG", "CD", "OE1", ["OE2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CGLY" => [
            ("C", "CA", "N", ["HA2", "HA3"]),
            ("N", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CHID" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CE1", "ND1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "NE2", false),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CHIE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CB", "CG", "CD2", "NE2", false),
            ("CD2", "NE2", "CE1", ["HE2"]),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CHIP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CE1", "ND1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "NE2", false),
            ("CD2", "NE2", "CE1", ["HE2"]),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CILE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG1", true),
            ("CA", "CB", "CG1", ["CG2", "HB"]),
            ("CA", "CB", "CG1", "CD1", true),
            ("CB", "CG1", "CD1", ["HG12", "HG13"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG1", "CD1", "HD11", true),
            ("CG1", "CD1", "HD11", ["HD12", "HD13"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CLEU" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2", "HG"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG", "CD2", "HD21", true),
            ("CG", "CD2", "HD21", ["HD22", "HD23"]),
            ("CB", "CG", "CD1", "HD11", true),
            ("CG", "CD1", "HD11", ["HD12", "HD13"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CLYS" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "CE", true),
            ("CE", "CD", "CG", ["HD2", "HD3"]),
            ("CG", "CD", "CE", "NZ", true),
            ("CD", "CE", "NZ", ["HE2", "HE3"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CD", "CE", "NZ", "HZ1", true),
            ("CE", "NZ", "HZ1", ["HZ2", "HZ3"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CMET" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CA", "CB", "CG", "SD", true),
            ("CB", "CG", "SD", ["HG2", "HG3"]),
            ("CB", "CG", "SD", "CE", true),
            ("CG", "SD", "CE", "HE1", true),
            ("SD", "CE", "HE1", ["HE2", "HE3"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CPHE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG", "CD1", "CE1", false),
            ("CE1", "CD1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["HD2"]),
            ("CG", "CD1", "CE1", "CZ", false),
            ("CD2", "CE2", "CZ", ["HE2"]),
            ("CE1", "CZ", "CE2", ["HZ"]),
            ("CD1", "CE1", "CZ", ["HE1"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CPRO" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("-C", "CA", "N", "CD", false),
            ("CA", "N", "CD", "CG", false),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CG", "CD", "N", ["HD2", "HD3"]),
        ],
    "CSER" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "OG", true),
            ("CA", "CB", "OG", ["HB2", "HB3"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CA", "CB", "OG", "HG", true),
            ("-C", "CA", "N", "H", false),
        ],
    "CTHR" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "OG1", true),
            ("CA", "CB", "OG1", ["CG2", "HB"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("CA", "CB", "OG1", "HG1", true),
            ("-C", "CA", "N", "H", false),
        ],
    "CTRP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["CE3"]),
            ("CE2", "CD2", "CE3", "CZ3", false),
            ("CD2", "CE3", "CZ3", ["HE3"]),
            ("CD2", "CE3", "CZ3", "CH2", false),
            ("CE3", "CZ3", "CH2", ["HZ3"]),
            ("CE3", "CD2", "CE2", "CZ2", false),
            ("CE2", "CZ2", "CH2", ["HZ2"]),
            ("CZ3", "CH2", "CZ2", ["HH2"]),
            ("CD2", "CE2", "CZ2", ["NE1"]),
            ("CG", "CD1", "NE1", ["HD1"]),
            ("CD1", "NE1", "CE2", ["HE1"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CTYR" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CB", "CG", "CD1", "CE1", false),
            ("CE1", "CD1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["HD2"]),
            ("CG", "CD1", "CE1", "CZ", false),
            ("CD2", "CE2", "CZ", ["HE2"]),
            ("CE1", "CZ", "CE2", ["OH"]),
            ("CD1", "CE1", "CZ", ["HE1"]),
            ("CE1", "CZ", "OH", "HH", true),
            ("-C", "CA", "N", "H", false),
        ],
    "CVAL" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG1", true),
            ("CA", "CB", "CG1", ["CG2", "HB"]),
            ("CB", "CA", "C", "OXT", true),
            ("CA", "C", "OXT", ["O"]),
            ("CA", "CB", "CG1", "HG11", true),
            ("CB", "CG1", "HG11", ["HG12", "HG13"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("-C", "CA", "N", "H", false),
        ],
    "CYS" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "SG", true),
            ("CA", "CB", "SG", ["HB2", "HB3"]),
            ("CA", "CB", "SG", "HG", true),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "GLN" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "OE1", true),
            ("CG", "CD", "OE1", ["NE2"]),
            ("CG", "CD", "NE2", "HE21", true),
            ("CD", "NE2", "HE21", ["HE22"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "GLU" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "OE1", true),
            ("CG", "CD", "OE1", ["OE2"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "GLY" => [
            ("C", "CA", "N", ["HA2", "HA3"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "HID" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CE1", "ND1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "NE2", false),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "HIE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CB", "CG", "CD2", "NE2", false),
            ("CD2", "NE2", "CE1", ["HE2"]),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "HIP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CE1", "ND1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "NE2", false),
            ("CD2", "NE2", "CE1", ["HE2"]),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "ILE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG1", true),
            ("CA", "CB", "CG1", ["CG2", "HB"]),
            ("CA", "CB", "CG1", "CD1", true),
            ("CB", "CG1", "CD1", ["HG12", "HG13"]),
            ("CB", "CG1", "CD1", "HD11", true),
            ("CG1", "CD1", "HD11", ["HD12", "HD13"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "LEU" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2", "HG"]),
            ("CB", "CG", "CD2", "HD21", true),
            ("CG", "CD2", "HD21", ["HD22", "HD23"]),
            ("CB", "CG", "CD1", "HD11", true),
            ("CG", "CD1", "HD11", ["HD12", "HD13"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "LYS" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "CE", true),
            ("CE", "CD", "CG", ["HD2", "HD3"]),
            ("CG", "CD", "CE", "NZ", true),
            ("CD", "CE", "NZ", ["HE2", "HE3"]),
            ("CD", "CE", "NZ", "HZ1", true),
            ("CE", "NZ", "HZ1", ["HZ2", "HZ3"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "MET" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "SD", true),
            ("CB", "CG", "SD", ["HG2", "HG3"]),
            ("CB", "CG", "SD", "CE", true),
            ("CG", "SD", "CE", "HE1", true),
            ("SD", "CE", "HE1", ["HE2", "HE3"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "NALA" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("C", "CA", "CB", "HB1", true),
            ("CA", "CB", "HB1", ["HB2", "HB3"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NARG" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "NE", true),
            ("CG", "CD", "NE", ["HD2", "HD3"]),
            ("CG", "CD", "NE", "CZ", true),
            ("CD", "NE", "CZ", ["HE"]),
            ("CD", "NE", "CZ", "NH1", true),
            ("NE", "CZ", "NH1", ["NH2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("+N", "CA", "C", "O", false),
            ("NE", "CZ", "NH2", "HH22", false),
            ("CZ", "NH2", "HH22", ["HH21"]),
            ("NE", "CZ", "NH1", "HH11", false),
            ("CZ", "NH1", "HH11", ["HH12"]),
        ],
    "NASN" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "OD1", true),
            ("CB", "CG", "OD1", ["ND2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG", "ND2", "HD21", true),
            ("CG", "ND2", "HD21", ["HD22"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NASP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "OD1", true),
            ("CB", "CG", "OD1", ["OD2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NCYS" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "SG", true),
            ("CA", "CB", "SG", ["HB2", "HB3"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CA", "CB", "SG", "HG", true),
            ("+N", "CA", "C", "O", false),
        ],
    "NGLN" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "OE1", true),
            ("CG", "CD", "OE1", ["NE2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CG", "CD", "NE2", "HE21", true),
            ("CD", "NE2", "HE21", ["HE22"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NGLU" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "OE1", true),
            ("CG", "CD", "OE1", ["OE2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NGLY" => [
            ("C", "CA", "N", ["HA2", "HA3"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NHID" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CE1", "ND1", "CG", ["HD1"]),
            ("CG", "ND1", "CE1", "NE2", false),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NHIE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CG", "ND1", "CE1", "NE2", false),
            ("CD2", "NE2", "CE1", ["HE2"]),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NHIP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "ND1", true),
            ("CB", "CG", "ND1", ["CD2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG", "ND1", "CE1", false),
            ("CE1", "ND1", "CG", ["HD1"]),
            ("CG", "ND1", "CE1", "NE2", false),
            ("CD2", "NE2", "CE1", ["HE2"]),
            ("CG", "CD2", "NE2", ["HD2"]),
            ("ND1", "CE1", "NE2", ["HE1"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NILE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG1", true),
            ("CA", "CB", "CG1", ["CG2", "HB"]),
            ("CA", "CB", "CG1", "CD1", true),
            ("CB", "CG1", "CD1", ["HG12", "HG13"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG1", "CD1", "HD11", true),
            ("CG1", "CD1", "HD11", ["HD12", "HD13"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NLEU" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2", "HG"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG", "CD2", "HD21", true),
            ("CG", "CD2", "HD21", ["HD22", "HD23"]),
            ("CB", "CG", "CD1", "HD11", true),
            ("CG", "CD1", "HD11", ["HD12", "HD13"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NLYS" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD", true),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CB", "CG", "CD", "CE", true),
            ("CE", "CD", "CG", ["HD2", "HD3"]),
            ("CG", "CD", "CE", "NZ", true),
            ("CD", "CE", "NZ", ["HE2", "HE3"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CD", "CE", "NZ", "HZ1", true),
            ("CE", "NZ", "HZ1", ["HZ2", "HZ3"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NMET" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "SD", true),
            ("CB", "CG", "SD", ["HG2", "HG3"]),
            ("CB", "CG", "SD", "CE", true),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CG", "SD", "CE", "HE1", true),
            ("SD", "CE", "HE1", ["HE2", "HE3"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NPHE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG", "CD1", "CE1", false),
            ("CE1", "CD1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["HD2"]),
            ("CG", "CD1", "CE1", "CZ", false),
            ("CD2", "CE2", "CZ", ["HE2"]),
            ("CE1", "CZ", "CE2", ["HZ"]),
            ("CD1", "CE1", "CZ", ["HE1"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NPRO" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["CD", "H3"]),
            ("CA", "N", "CD", "CG", false),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CG", "CD", "N", ["HD2", "HD3"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NSER" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "OG", true),
            ("CA", "CB", "OG", ["HB2", "HB3"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CA", "CB", "OG", "HG", true),
            ("+N", "CA", "C", "O", false),
        ],
    "NTHR" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "OG1", true),
            ("CA", "CB", "OG1", ["CG2", "HB"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("CA", "CB", "OG1", "HG1", true),
            ("+N", "CA", "C", "O", false),
        ],
    "NTRP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["CE3"]),
            ("CE2", "CD2", "CE3", "CZ3", false),
            ("CD2", "CE3", "CZ3", ["HE3"]),
            ("CD2", "CE3", "CZ3", "CH2", false),
            ("CE3", "CZ3", "CH2", ["HZ3"]),
            ("CE3", "CD2", "CE2", "CZ2", false),
            ("CE2", "CZ2", "CH2", ["HZ2"]),
            ("CZ3", "CH2", "CZ2", ["HH2"]),
            ("CD2", "CE2", "CZ2", ["NE1"]),
            ("CG", "CD1", "NE1", ["HD1"]),
            ("CD1", "NE1", "CE2", ["HE1"]),
            ("+N", "CA", "C", "O", false),
        ],
    "NTYR" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CB", "CG", "CD1", "CE1", false),
            ("CE1", "CD1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["HD2"]),
            ("CG", "CD1", "CE1", "CZ", false),
            ("CD2", "CE2", "CZ", ["HE2"]),
            ("CE1", "CZ", "CE2", ["OH"]),
            ("CD1", "CE1", "CZ", ["HE1"]),
            ("CE1", "CZ", "OH", "HH", true),
            ("+N", "CA", "C", "O", false),
        ],
    "NVAL" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG1", true),
            ("CA", "CB", "CG1", ["CG2", "HB"]),
            ("C", "CA", "N", "H2", true),
            ("CA", "N", "H2", ["H1", "H3"]),
            ("CA", "CB", "CG1", "HG11", true),
            ("CB", "CG1", "HG11", ["HG12", "HG13"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("+N", "CA", "C", "O", false),
        ],
    "PHE" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("CB", "CG", "CD1", "CE1", false),
            ("CE1", "CD1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["HD2"]),
            ("CG", "CD1", "CE1", "CZ", false),
            ("CD2", "CE2", "CZ", ["HE2"]),
            ("CE1", "CZ", "CE2", ["HZ"]),
            ("CD1", "CE1", "CZ", ["HE1"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "PRO" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("-C", "CA", "N", "CD", false),
            ("CA", "N", "CD", "CG", false),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CG", "CD", "N", ["HD2", "HD3"]),
            ("CB", "CG", "CD", ["HG2", "HG3"]),
            ("+N", "CA", "C", "O", false),
        ],
    "SER" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "OG", true),
            ("CA", "CB", "OG", ["HB2", "HB3"]),
            ("CA", "CB", "OG", "HG", true),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "THR" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "OG1", true),
            ("CA", "CB", "OG1", ["CG2", "HB"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("CA", "CB", "OG1", "HG1", true),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "TRP" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["CE3"]),
            ("CE2", "CD2", "CE3", "CZ3", false),
            ("CD2", "CE3", "CZ3", ["HE3"]),
            ("CD2", "CE3", "CZ3", "CH2", false),
            ("CE3", "CZ3", "CH2", ["HZ3"]),
            ("CE3", "CD2", "CE2", "CZ2", false),
            ("CE2", "CZ2", "CH2", ["HZ2"]),
            ("CZ3", "CH2", "CZ2", ["HH2"]),
            ("CD2", "CE2", "CZ2", ["NE1"]),
            ("CG", "CD1", "NE1", ["HD1"]),
            ("CD1", "NE1", "CE2", ["HE1"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "TYR" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG", true),
            ("CA", "CB", "CG", ["HB2", "HB3"]),
            ("CA", "CB", "CG", "CD1", true),
            ("CB", "CG", "CD1", ["CD2"]),
            ("CB", "CG", "CD1", "CE1", false),
            ("CE1", "CD1", "CG", ["HD1"]),
            ("CB", "CG", "CD2", "CE2", false),
            ("CE2", "CD2", "CG", ["HD2"]),
            ("CG", "CD1", "CE1", "CZ", false),
            ("CD2", "CE2", "CZ", ["HE2"]),
            ("CE1", "CZ", "CE2", ["OH"]),
            ("CD1", "CE1", "CZ", ["HE1"]),
            ("CE1", "CZ", "OH", "HH", true),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
    "VAL" => [
            ("C", "CA", "N", ["CB", "HA"]),
            ("C", "CA", "CB", "CG1", true),
            ("CA", "CB", "CG1", ["CG2", "HB"]),
            ("CA", "CB", "CG1", "HG11", true),
            ("CB", "CG1", "HG11", ["HG12", "HG13"]),
            ("CA", "CB", "CG2", "HG21", true),
            ("CB", "CG2", "HG21", ["HG22", "HG23"]),
            ("+N", "CA", "C", "O", false),
            ("-C", "CA", "N", "H", false),
        ],
)

# Whether a residue's own phi dihedral (about its N-CA bond) is a free
# parameter. It is fixed when N-CA lies on a ring, e.g. proline's; the
# ring's fixed bond lengths and angles then leave phi no freedom to vary.
const residue_phi_rotatable = Dict(
    "ALA" => true,
    "ARG" => true,
    "ASN" => true,
    "ASP" => true,
    "CALA" => true,
    "CARG" => true,
    "CASN" => true,
    "CASP" => true,
    "CCYS" => true,
    "CGLN" => true,
    "CGLU" => true,
    "CGLY" => true,
    "CHID" => true,
    "CHIE" => true,
    "CHIP" => true,
    "CILE" => true,
    "CLEU" => true,
    "CLYS" => true,
    "CMET" => true,
    "CPHE" => true,
    "CPRO" => false,
    "CSER" => true,
    "CTHR" => true,
    "CTRP" => true,
    "CTYR" => true,
    "CVAL" => true,
    "CYS" => true,
    "GLN" => true,
    "GLU" => true,
    "GLY" => true,
    "HID" => true,
    "HIE" => true,
    "HIP" => true,
    "ILE" => true,
    "LEU" => true,
    "LYS" => true,
    "MET" => true,
    "NALA" => true,
    "NARG" => true,
    "NASN" => true,
    "NASP" => true,
    "NCYS" => true,
    "NGLN" => true,
    "NGLU" => true,
    "NGLY" => true,
    "NHID" => true,
    "NHIE" => true,
    "NHIP" => true,
    "NILE" => true,
    "NLEU" => true,
    "NLYS" => true,
    "NMET" => true,
    "NPHE" => true,
    "NPRO" => false,
    "NSER" => true,
    "NTHR" => true,
    "NTRP" => true,
    "NTYR" => true,
    "NVAL" => true,
    "PHE" => true,
    "PRO" => false,
    "SER" => true,
    "THR" => true,
    "TRP" => true,
    "TYR" => true,
    "VAL" => true,
)
